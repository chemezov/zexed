...
class Image extends YModel
{
	...
	public function rules()
	{
		return array(
			...
			array('sort, status, parent_id, type, category_id', 'numerical', 'integerOnly' => true),
			...
			array('id, name, description, creation_date, user_id, alt, status, galleryId, sort', 'safe', 'on' => 'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'sort'          => Yii::t('ImageModule.image', 'Сортировка'),
		);
	}
	...
	public function makeThumbnail($width = 0, $height = 0)
	{
		...
		if (file_exists($image->getUploadPath() . $file) === false) {
			$thumb = Yii::app()->image->load($image->getUploadPath() . $this->file);
//            $thumb = Yii::app()->thumbs->create($image->getUploadPath() . $this->file);
			$thumb->smartResize($width, $height);
			$thumb->quality(100)->save($image->getUploadPath() . $file);
		}
	...
	{
		if ($this->isNewRecord) {
			$max = Yii::app()->db->createCommand('SELECT MAX(sort) FROM {{image_image}}')->queryScalar();
